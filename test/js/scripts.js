...
( function() {
    document.addEventListener( "DOMContentLoaded", function() {
        ...
        document.getElementById( "header" ).innerText += " - v" + $rink.getVersion();
        ...
        _IGNORE_ELEMENT_IDS = document.getElementById( "ignoreElementIds" );
...
function getJSONData() {
    ...
    _JSON_TEXTAREA.value = $rink
        ...
        .ignoreElementIds( _IGNORE_ELEMENT_IDS.value )
...
function getJSONVariables() {
    var variables = $rink.json().getVariables( "{{variable_start}}<div>{{variable_1}}</div><div>{{variable_2}}</div>{{variable_end}}" ),
...
function getHTMLVariables() {
    var variables = $rink.html().getVariables( document.getElementById( "description" ) ),
        variablesString = JSON.stringify( variables );
...
function getVersion() {
    _JSON_TEXTAREA.value = $rink.getVersion();
